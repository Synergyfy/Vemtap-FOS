      ...
      <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-lg shadow-xl">
        ...
        {payload.map((entry: TooltipPayloadEntry, index: number) => (
          <p key={index} className="text-sm font-semibold" style={{ color: entry.color || entry.fill }}>
            {entry.name}: {formatNaira(Number(entry.value))}
          </p>
        ))}
      </div>
    );
...
export default function RevenueOverviewPage() {
  ...
  const revenueCards = useMemo(() => {
    ...
    return [
      ...
      { label: "Messaging Revenue", value: a ? formatNaira(a.smsRevenue) : "—", icon: MessageSquare, color: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-900/30" },
      { label: "QRThrive Revenue", value: "—", icon: QrCode, color: "text-purple-500", bg: "bg-purple-50 dark:bg-purple-900/30" },
      { label: "Hardware Revenue", value: "—", icon: Monitor, color: "text-indigo-500", bg: "bg-indigo-50 dark:bg-indigo-900/30" },
      { label: "Enterprise Revenue", value: "—", icon: Building2, color: "text-cyan-500", bg: "bg-cyan-50 dark:bg-cyan-900/30" },
    ];
  }, [aggregates]);
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50 flex items-center">
            Revenue Overview
            <InfoTooltip content="Track all money coming into VEMTAP." />
          </h1>
          <p className="text-zinc-500">Every source of revenue across the entire platform.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-1.5">
            ...
            <select value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setCurrentPage(1); }}
              ...
              <option value="SMS">Messaging</option>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-6">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50 mb-6 flex items-center gap-2">
            <BarChart3 className="w-5 h-5 text-indigo-500" /> Revenue by Source
            <InfoTooltip content="Compare revenue across platforms." />
          </h2>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-6">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50 mb-6 flex items-center gap-2">
            ...
            <InfoTooltip content="See which revenue types contribute the most." />
          </h2>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={planData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                    {planData.map((_: unknown, index: number) => (
                      <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip formatter={(value: unknown) => formatNaira(Number(value))} />
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        <div className="border-b border-zinc-200 dark:border-zinc-800 p-6 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50 flex items-center">
            ...
            <InfoTooltip content="Every individual income transaction." />
          </h2>
