...
export default function AgentsPage() {
  ...
  const { data: backendData, error, isLoading } = useAgents();
  const { data: revenueData, isLoading: revenueLoading } = useAgentRevenue(
  ...
  const rawUsers = useMemo(() => {
    ...
    return backendData.agents.map((agent: any) => ({
      ...
      email: agent.email,
      phone: agent.phone || "",
      role: agent.managerId ? "Agent" : "Manager",
      businesses: agent.businessesCount || 0,
      rawRevenue: agent.managedMrr || 0,
      conversion: agent.businessesCount > 0 ? Number(((agent.businessesCount / 12) * 10).toFixed(1)) : 0,
      status: agent.status === "ACTIVE" ? "Active" : agent.status === "SUSPENDED" ? "Suspended" : "Inactive",
      managerId: agent.managerId || undefined,
    }));
  }, [backendData]);
  ...
  const { processedUsers, summaryStats } = useMemo(() => {
    ...
    const agents = rawOnlyAgents.map((agent: any) => {
      ...
      return {
        ...
        bonusBreakdown: bonuses.map((b) => ({
          name: b.name,
          amount:
            b.type === "percent" ? agent.rawRevenue * (b.value / 100) : b.value,
        })),
      };
    });
    ...
    const stats = [
      ...
      },
      {
        ...
        value: formatNaira(totalAgentCommissions),
        icon: Users,
        color: "text-green-500",
        bg: "bg-green-50 dark:bg-green-900/30",
      },
      {
        label: "Manager Commissions",
        value: formatNaira(totalManagerCommissions),
        icon: UserCheck,
        color: "text-purple-500",
        bg: "bg-purple-50 dark:bg-purple-900/30",
      },
      {
        label: "Total Payout (Inc. Bonuses)",
        value: formatNaira(
          totalAgentCommissions + totalManagerCommissions + totalBonusesPaid,
        ),
        icon: Wallet,
  ...
  }, [agentRate, managerRate, bonuses, roleFilter, rawUsers]);
  ...
  const generatePieData = (agent: any) => {
    if (agent.role === "Manager") {
      return [
        {
          name: `Manager Cut (${managerRate}%)`,
          value: agent.managerCommission,
        },
      ];
    }
    const data = [
      {
        name: `Base Commission (${agentRate}%)`,
  ...
  if (error) {
    return (
      <div className="p-6 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-900/50 rounded-xl text-sm font-medium">
        Failed to load CRM agents. Please verify that the API is online.
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-8 relative">
      <div>
        ...
        <p className="text-zinc-500">
          Track performance and dynamically adjust compensation structures.
        </p>
      </div>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                step="0.5"
                value={managerRate}
                onChange={(e) => setManagerRate(Number(e.target.value))}
              />
            </div>
            ...
            <div className="p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg flex items-center justify-between border border-zinc-100 dark:border-zinc-800">
              <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400">
                Total Subscription Split:
              </span>
              <span className="text-lg font-bold text-zinc-900 dark:text-zinc-50">
                {agentRate + managerRate}%
              </span>
            </div>
          </div>
            ...
            {bonuses.length === 0 ? (
              ...
              <div className="space-y-3">
                {bonuses.map((bonus) => (
                  <div
                    key={bonus.id}
                    className="flex flex-wrap sm:flex-nowrap items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-lg"
                  >
                    <input
                      type="text"
                      value={bonus.name}
                    ...
                    <select
                      value={bonus.type}
                      onChange={(e) =>
                        updateBonus(bonus.id, "type", e.target.value)
                      }
        ...
        <div className="border-b border-zinc-200 dark:border-zinc-800 p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-zinc-500">
              {processedUsers.length} members total
            </span>
          </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-zinc-500 uppercase bg-zinc-50 dark:bg-zinc-950/50 border-b border-zinc-200 dark:border-zinc-800">
              <tr>
                ...
                <th className="px-6 py-4 font-medium text-right bg-green-50/50 dark:bg-green-900/10">
                  Agent Payout
                </th>
                <th className="px-6 py-4 font-medium text-right bg-purple-50/50 dark:bg-purple-900/10">
                  Manager Cut
                </th>
              ...
              {processedUsers.map((user: any) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right font-medium text-green-700 dark:text-green-400 bg-green-50/30 dark:bg-green-900/5">
                    {user.role === "Manager"
                      ? "-"
                      : formatNaira(user.agentTotalPayout)}
                    {user.role !== "Manager" && bonuses.length > 0 && (
                      <span title="Includes bonuses">
                        <Gift className="w-3 h-3 inline-block ml-1 text-amber-500" />
                      </span>
                    )}
                  </td>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedAgent.role === "Manager" && (
                  <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-zinc-100 dark:border-zinc-800">
                    ...
                    <p className="text-xl font-bold text-zinc-900 dark:text-zinc-50">
                      {selectedAgent.recruitCount}
                    </p>
                  </div>
                )}
                <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-zinc-100 dark:border-zinc-800">
                  <p className="text-xs font-medium text-zinc-500 mb-1 flex items-center gap-1">
                    <Building2 className="w-3 h-3" /> Acquired
                  </p>
                  <p className="text-xl font-bold text-zinc-900 dark:text-zinc-50">
                    {selectedAgent.businesses}
                  </p>
                </div>
                ...
                {selectedAgent.role === "Agent" ? (
                  <div className="col-span-2 bg-green-50 dark:bg-green-900/10 p-4 rounded-xl border border-green-100 dark:border-green-900/30">
                    <p className="text-xs font-medium text-green-700 dark:text-green-400 mb-1 flex items-center gap-1">
                      <Wallet className="w-3 h-3" /> Total Agent Earnings
                    </p>
                    <p className="text-2xl font-bold text-green-900 dark:text-green-500">
                      {formatNaira(selectedAgent.agentTotalPayout)}
                    </p>
                    {bonuses.length > 0 && (
                      <p className="text-xs text-green-600 dark:text-green-500 mt-1 flex items-center gap-1">
                        <Gift className="w-3 h-3" /> Includes {bonuses.length}{" "}
                        active bonus(es)
                      </p>
                    )}
                  </div>
                  ...
                  <div className="col-span-2 bg-purple-50 dark:bg-purple-900/10 p-4 rounded-xl border border-purple-100 dark:border-purple-900/30">
                    <p className="text-xs font-medium text-purple-700 dark:text-purple-400 mb-1 flex items-center gap-1">
                      <Wallet className="w-3 h-3" /> Total Manager Cut Earned
                    </p>
                    <p className="text-2xl font-bold text-purple-900 dark:text-purple-500">
                      {formatNaira(selectedAgent.managerCommission)}
                    </p>
                  </div>
                )}
              </div>
